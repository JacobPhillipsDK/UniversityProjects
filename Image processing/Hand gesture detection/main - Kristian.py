...
def erotion(threshold):
    ...
    for y in range(threshold.shape[0]):
        for x in range(threshold.shape[1]):
            ...
            if np.all(hit == croparray) and x > 1 and y > convex and x < threshold.shape[1]-1 and y<threshold.shape[0]-1:
                newArray[y][x] = 255
            elif y <= convex:
...
def dilation(threshold):
    ...
    for y in range(threshold.shape[0]):
        for x in range(threshold.shape[1]):
            crop = threshold[y:y+3,x:x+3]
            croparray = np.array(crop)
            if np.any(hit == croparray) and x > 1 and y > convex and x < threshold.shape[1]-1 and y<threshold.shape[0]-1:
                newArray[y][x] = 255
                newArray[y][x+1] = 255
...
def convexity(maxContour):
    ...
    if len(farPoints)>1:
        ...
        print("Didn't find enough farPoints")
        return 200
    ...
    if cv2.waitKey(1) & 0xFF == ord('e'):
        ...
        for i in range(2):
            if len(contours) > 0:
                ...
                (_, contours1, _) = cv2.findContours(thresholderode, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
                ...
                for i in range(2):
                    hull3.append(cv2.convexHull(maxContour1))
                    cv2.drawContours(drawing2, hull3, i, color, 1, 8)
                ...
                cv2.putText(diff,"No hands found",(3,100),cv2.FONT_HERSHEY_COMPLEX,0.7,(255,255,255),1)
