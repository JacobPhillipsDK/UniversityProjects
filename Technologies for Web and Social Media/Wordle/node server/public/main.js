let CorrectWord = String("SPACE");

function setWordFromHttpRequest(response) { // Sets the correct word to the response text
    CorrectWord = String(response.toUpperCase())
}
...
function resetGame() {
    GuessRemaining = NUMBER_OF_ROWS
    nextLetter = 0
    currentGuess = []
    let row = document.getElementsByClassName("letter-row")
    for (let i = 0; i < row.length; i++) {
        let box = row[i].children
        for (let j = 0; j < box.length; j++) {
            box[j].textContent = ""
            box[j].classList.remove("filled-box")
            box[j].classList.remove("correctLetter")
            box[j].classList.remove("correctLetterPlacement")
            box[j].classList.remove("wrongLetterGuess")
        }
    }
}
...
function getWordRequest() {
    const xhttp = new XMLHttpRequest();
    xhttp.onload = function () {
        console.log("this.responseText: " + this.responseText)
        const wordObj = JSON.parse(this.responseText)
        console.log("wordObj: " + wordObj.word)
        setWordFromHttpRequest(wordObj.word)
    };
    xhttp.open("GET", "http://localhost:3000/getword", true);
    xhttp.send();
}
